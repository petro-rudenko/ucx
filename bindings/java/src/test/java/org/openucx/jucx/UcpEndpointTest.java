...
public class UcpEndpointTest extends UcxTest {
    ...
    public void testStreamingAPI() {
        UcpContext context1 = new UcpContext(new UcpParams().requestStreamFeature().requestRmaFeature());
        UcpContext context2 = new UcpContext(new UcpParams().requestStreamFeature().requestRmaFeature());
        ...
        UcpRequest recvRequest = serverToClient.recvStreamNonBlocking(UcxUtils.getAddress(recvBuffer),
          UcpMemoryTest.MEM_SIZE, 0, new UcxCallback() {
              @Override
              public void onSuccess(UcpRequest request) {
                  received.set(true);
              }
