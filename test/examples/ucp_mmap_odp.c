...
static long block_size = 1024*1024; //1m Size of block granularity for mmap
static long total_size = 1024*1024*1024*1024; //1Tb total file size
static ucp_mem_h *ucp_mem_handles = malloc(sizeof(ucp_mem_h) * (total_size / block_size));
...
static int run_ucx_server(ucp_worker_h ucp_worker)
{
    ...
    msg_len = sizeof(*msg) + block_size;
    msg = calloc(1, msg_len);
    CHKERR_JUMP(!msg, "allocate memory\n", err_ep);

    msg->data_len = msg_len - sizeof(*msg);
    generate_test_string((char *)(msg + 1), block_size);
...
static inline long round_to_next_page_boundary(off_t i) {
    return (i + getpagesize() - 1) & ~(getpagesize() - 1);
  }


static void do_mmap_and_register(ucp_context_h context)
{
    FILE* data = fopen("data", "r");
    int fd = fileno(data);
    ucp_mem_map_params_t mem_map_params;
    ucp_mem_map_params_t.field_mask = UCP_MEM_MAP_PARAM_FIELD_ADDRESS |
                                      UCP_MEM_MAP_PARAM_FIELD_LENGTH |
                                      UCP_MEM_MAP_PARAM_FIELD_FLAGS;
    ucp_mem_map_params_t.length = block_size;
    ucp_mem_map_params_t.flags = UCP_MEM_MAP_NONBLOCK | UCP_MEM_MAP_FIXED;
    size_t total_mem_regions = 0;
    off_t offset = 0;
    size_t i = 0;
    while (total_mem_regions < total_size)
    {
        void *addr = mmap(NULL, block_size, PROT_READ, MAP_SHARED, fd, offset);
        offset += round_to_next_page_boundary(offset + block_size);
        ucp_mem_map_params_t.address = addr;
        ucp_mem_h mem_h;
        ucp_mem_map(context, ucp_mem_map_params_t, ucp_mem_handles + i * sizeof(mem_h));
        i++;
    }
...
int main(int argc, char **argv)
{
    ...
    do_mmap_and_register(ucp_context);
...
int parse_cmd(int argc, char * const argv[], char **server_name)
{
    ...
    while ((c = getopt(argc, argv, "wfben:p:s:t:h")) != -1) {
        switch (c) {
        case 'w':
        ...
        case 's':
         block_size = atol(optarg);
            if  test_block_size <= 0) {
                fprintf(stderr, "Wrong block size %ld\n", block_size);
                return UCS_ERR_UNSUPPORTED;
            }
            break;
        case 't':
        ...
        default:
            ...
            fprintf(stderr, "  -s size  Set block size (default:1m)\n");
            fprintf(stderr, "  -t total Length to do mmap blocks (default:1b)\n");
            fprintf(stderr, "\n");
            return UCS_ERR_UNSUPPORTED;
        }
    }
